...
import { isWhitespaceSensitiveElement, isWhitespaceNode } from '../nodeUtils.js';
...
export default (code) => {
  ...
  /** @type {import('@keithclark/tiny-parsers').SgmlNode} */
  let node;

  for (node of walkTree(fragment)) {
...
const minify = (fragment) => {
  ...
  const min = (node) => {
    const clone = {...node};
    if (node.type === NODE_TYPE_ELEMENT) {
      if (isWhitespaceSensitiveElement(node)) {
        return clone;
      }
      clone.children = node.children.map(min).filter(x => !!x)
    } else if (isWhitespaceNode(node)) {
      return null;
    } else if (node.type === NODE_TYPE_TEXT) {
      clone.value = node.value.replace(/\s+/g,' ');
